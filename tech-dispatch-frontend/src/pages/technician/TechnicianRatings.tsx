...
export default function TechnicianRatings() {
  ...
  useEffect(() => {
    const fetch = async () => {
      try {
        const data = await technicianService.getRatings();
        setRatings(data);
      } catch {
                ...
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <p className="text-sm font-medium text-dark">
                      {typeof rating.userId === "object" && rating.userId ? rating.userId.name : "Anonymous"}
                    </p>
                    <p className="text-xs text-gray-400">
                      {typeof rating.bookingId === "object" && rating.bookingId ? rating.bookingId.serviceType : "Service"} &middot;{" "}
