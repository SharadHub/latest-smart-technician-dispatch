...
export default function TechnicianDashboard() {
  ...
  useEffect(() => {
    ...
    socket.emit("join", { userId: user.id, role: "technician" });

    socket.on("booking-request", (payload: { bookingId: string; serviceType: string; expiresAt: number }) => {
      addBooking(payload);
  ...
  }, [token, user, addBooking, removeBooking]);

  // Fetch availability
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        const res = await api.get("/technicians/profile");
        setIsAvailable(res.data.data.status === "active");
      } catch {
        // not yet approved
      }
    };
